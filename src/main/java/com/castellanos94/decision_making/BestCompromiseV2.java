...
public class BestCompromiseV2 {
    ...
    public ArrayList<DoubleSolution> execute() throws FileNotFoundException {
        ...
        if (indexCandidatos.size() == 1) {
            ...
            for (Integer index : indexCandidatos) {
                RealData tmp = (RealData) sample.get(index).getAttribute(NETSCORE_KEY);
        ...
        if (indexWeakness != -1) {
            ...
            best = (RealData) best_compromise.getAttribute(NETSCORE_KEY);
        } else {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Solution.writSolutionsToFile("bestCompromise" + File.separator + "bestCompromiseC_" + problem.getName(),
                new ArrayList<>(roi));
    }
}
